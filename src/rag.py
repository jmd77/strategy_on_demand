...
def generate_response(
    *,
    ...
    full_prompt = build_prompt(use_case=use_case, briefing=briefing, extra_context=extra_context)
    ...
    for item in outputs:
        if isinstance(item, dict):
            ...
            item_contents = item.get("content") or []
        else:
            item_type = getattr(item, "type", None)
            item_contents = getattr(item, "content", []) or []
        ...
        for content in item_contents:
            ...
            if content_type in {"text", "input_text", "output_text"} and isinstance(value, str):
                cleaned = value.strip()
                if cleaned:
    ...
    raise RuntimeError("No textual content returned by the Responses API call.")
